...
**One verifiable gap.** The IMPRESS abstract says the method "can be
...
and two `ls`. That gap looks to us like the most promising direction for improvement, and it is
where we expect our own work to go next.
...
- **The perturbation is invisible by construction**, so you cannot look for it. It has to be
  found by changing coordinates — which is what the radial power-spectrum analysis in our
  notebook does. *(Built and working; not yet run against the results below, so we quote no
  number from it here.)*
...
- **Ranking budget: LPIPS ≤ 0.10** — the outer bound for being *ranked at all*; over-budget
  entries are still plotted and discussed, just not ranked. **The tighter 0.017 in the win
  condition is a different thing**: it is what the baseline actually spends. So ≤ 0.10 gets you
  on the board; ≤ 0.017 with `R_pipe` ≤ 0 beats the published defence.
...
**Submissions close 18 September**, with every result published in our Milestone 2 post —
**including ours losing.** Our purifier is deliberately blind; if your protection defeats it,
that is a real result about IMPRESS and we will report it as one, not quietly drop it.
...
- **Two different quantities in this project are called SSIM.** `pg_metric.py` compares *edited*
  ...
  photograph intact?" The fidelity figures quoted in §4 — LPIPS and mean
  absolute difference against the original photograph — are the second quantity. We label which is which
  every time.
...
**M2** — the `pg_eps` sweep (16 → 24 → 32) on ≥ 5 faces, our CLIPScore edit-success metric — built, and scored with an
independent CLIP model rather than the ViT-L/14 encoder inside Stable Diffusion, so we are not
grading the dish with the chef — run against real results for the first time, LPIPS reported alongside SSIM throughout, and a cheap-purifier ablation
